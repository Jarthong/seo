...
host = 'www.zbg.com'
server_id = 'excxxxxxxxxxx'
http_key = '71a80xxxxxxxxxxxxxxxxx3f'
...
market_id_list = get_market_id_list()
'''

# 推荐区市场列表 btc_usdt(329),eth_usdt(330），xrp_usdt(386),bchabc_usdt(415),ltc_usdt(331),usdt_qc(374),eos_usdt(333)
def get_market_id_list():
    market_id_list = ['336', '382', '380', '416', '417', '321', '364', '476', '466', '477', '5003', '5004', '5009', '5008']
    market_id_list_hot = ['329', '330', '386', '415', '331', '374', '333']
    num = random.randint(1, 10)
    if num <= 8:
        return market_id_list_hot
    else:
...
def get_price(market_id):
    try:
        ...
        if market_id == '329':
            return round(random.uniform(8500, 10000), 2)
        else:
...
def get_amount(market_id):
    ...
    elif market_id in ['330', '331']:
        return round(random.uniform(0, 20), 4)
    elif market_id == '386':
        return round(random.uniform(100, 2000), 2)
    elif market_id == '415':
        return round(random.uniform(0.5, 25), 4)
    elif market_id == '374':
        return round(random.uniform(10, 5000), 4)
    elif market_id == '333':
        return round(random.uniform(5, 200), 4)
    else:
        return round(random.uniform(1, 100), 2)
...
while True:
    ...
    market_id = random.choice(get_market_id_list())   # 随机选择一个市场id
